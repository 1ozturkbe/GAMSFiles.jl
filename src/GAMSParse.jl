...
function parseassignments(gams)
    ...
    if haskey(gams, "parameters") && haskey(gams, "assignments")
        parseassignments!(preexprs, gams["assignments"], getvars(gams["parameters"]), sets)
    end
    # Anyone using a solver (constrained optimization) approach might want to
    # skip the next part. It's designed to convert simple constraints into assignment statements.
    if haskey(gams, "equations")
        eqs2assigns!(bodyexprs, gams["equations"], vars, sets)
    end
    return preexprs, bodyexprs
end

function eqs2assigns!(bodyexprs, equations, vars, sets)
    sentinels = Dict()
    for (key, eq) in equations
        eq2assign!(bodyexprs, sentinels, eq, vars, sets)
    end
...
function eq2assign!(exprs, sentinels, eq, vars, sets)
    ...
    if lhs isa Union{GText,GArray} && haskey(vars, getname(lhs))
        ...
        if rhs isa GNumber && lhs isa GCall && lhs.name ∈ ("+", "-")
            ...
            while lhs isa GCall && lhs.name ∈ ("+", "-") && length(lhs.args) == 1
                # The equation is of the form -x =e= const"
                mul = lhs.name == "-" ? -1 : 1
                lhs = lhs.args[1]
                rhs = GNumber(mul * rhs.val)
                if mul == -1
                    op = flipop(op)
                end
            end
...
function parsegams(::Type{Module}, modname::Symbol, gams::Dict{String,Any})
    ...
    preexprs, bodyexprs = parseassignments(gams)
    # Create the return statement
    retvalstr = gams["minimizing"]
    push!(bodyexprs, :(return $(Symbol(retvalstr))))
    # As needed, destructure the input vector
    key, val = first(gams["variables"])  # Assume the first-listed variable is input
    if isa(key, GArray)
        xin = Symbol(getname(key))
    else
        # Assume that the only variables in the problem are inputs and retval
        varnames = Symbol[]
        for (key, val) in gams["variables"]
            if getname(key) != retvalstr
                push!(varnames, getname(key))
            end
        end
        xin = gensym("x")
    ...
    for p in ("parameters", "tables")
        if haskey(gams, p)
            for (k, v) in gams[p]
                push!(initexprs, Expr(:const, :($(getname(k)) = $v)))
            end
        end
    end
